...
def ingest_sqlite_dump(cursor, filename):
    ...
    for line in f.readlines():
        ...
        if sqlite3.complete_statement(sql):
            #sql = sql.strip()
            #Substiture Oracle TO_DATE to sqlite3 Date function
            sql_date = re.sub("TO_DATE", "Date", sql);
            sql = sql_date.strip()
            insert_or_ignore_sql  = re.sub("INSERT", "INSERT OR IGNORE", sql)
            sql = insert_or_ignore_sql
            create_table_if_not_exists = re.sub("CREATE TABLE", "CREATE TABLE IF NOT EXISTS", sql)
...
def ingest_epsg():
    ...
    for f in ['Oracle_Data_Script.sql',  'Oracle_Table_Script.sql']:
    ...
    ingest_sqlite_dump(cursor, 'Oracle_Table_Script.sql')
    ingest_sqlite_dump(cursor, 'Oracle_Data_Script.sql')
    cursor.close()
    conn.commit()
...
sql_dir_name = os.path.join(os.path.dirname(script_dir_name), 'data', 'sql_equinor_oracle')
...
for line in proj_db_conn.iterdump():
    if line.startswith('INSERT INTO "'):
        ...
        if table_name in files:
            ...
            f.write("--- This file has been generated by scripts/build_db_equinor_oracle.py. DO NOT EDIT !\n\n".encode('UTF-8'))
            files[table_name] = f
        f.write((line + '\n').encode('UTF-8'))
