...
static PJ_XY polyhedral_fwd(PJ_LP lp, PJ *P) {
    ...
    xy.x = (result.x - Q->x_offset) * Q->rq;
    xy.y = (result.y - Q->y_offset) * Q->rq;
...
static PJ_LP polyhedral_inv(PJ_XY xy, PJ *P) {
    ...
    const double x = xy.x / Q->rq + Q->x_offset;
    const double y = xy.y / Q->rq + Q->y_offset;
...
static PJ *polyhedral_setup(PJ *P, const polyhedral::PolyhedralDefaults &d) {
    ...
    if (P->es != 0.0) {
        ...
        // R_authalic / a, so a*rq == R_authalic after PROJ's core scaling.
        Q->rq = std::sqrt(0.5 * Q->qp);
    }
